import configparser, logging, json
...
if (zone_id is None or record_id is None):
    ...
    if zone_id:
        try:
            detailurl = '%s/%s/dns_records?name=%s' % (config['Cloudfare']['api_url'], zone_id, config['Cloudfare']['record_name'])
...
if zone_id and record_id:
    ...
    try:
        ...
        updaterq = Request(updateurl, data=content, method='PUT', headers=cf_headers)
